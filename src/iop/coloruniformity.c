...
typedef enum {
    ...
    PREVIEW_OVERLAY          = 4,
    PREVIEW_DELTA            = 5
} dt_iop_coloruniformity_preview_mode_t;

// --- 1. PARAMETERS AND INTROSPECTION (VERSION 12) ---

// clang-format off
typedef struct dt_iop_coloruniformity_params_t {
  // --- SELECTION (Source S) ---
  ...
  // --- SELECTION RANGE ---
  float hue_width;         // $MIN: 0.02 $MAX: 1.0 $DEFAULT: 0.30 $DESCRIPTION: "Hue Width"
  float shadow_limit;      // $MIN: 0.1 $MAX: 12.0 $DEFAULT: 1.5 $DESCRIPTION: "Shadow Limit"
  float highlight_limit;   // $MIN: 0.1 $MAX: 12.0 $DEFAULT: 1.5 $DESCRIPTION: "Highlight Limit"
  float mask_hardness;     // $MIN: 0.0 $MAX: 1.0 $DEFAULT: 0.0 $DESCRIPTION: "Mask Hardness"
  float mask_feather;      // $MIN: 0.0 $MAX: 1.0 $DEFAULT: 0.5 $DESCRIPTION: "Mask Feather"
  float chroma_gate;       // $MIN: 0.0 $MAX: 1.0 $DEFAULT: 1.0 $DESCRIPTION: "Chroma Gate"
...
} dt_iop_coloruniformity_params_t;
// clang-format on

DT_MODULE_INTROSPECTION(12, dt_iop_coloruniformity_params_t)
...
int legacy_params(dt_iop_module_t *self, const void *const old_params,
  ...
  if(old_version == 9)
  {
    // V9 -> V10: added bypass_hue, bypass_chroma, bypass_luma
    typedef struct {
      float source_hue, source_chroma, source_lightness;
      float hue_width, shadow_limit, highlight_limit;
      float target_hue, target_chroma, target_lightness;
      float strength_h, affinity_h, preserve_h;
      float strength_c, affinity_c, preserve_c;
      float strength_l, affinity_l, preserve_l;
      float offset_h, offset_c, offset_l;
      int   offsets_weighted;
      int   preview_mode;
    } dt_iop_coloruniformity_params_v9_t;

    const dt_iop_coloruniformity_params_v9_t *o = old_params;
    dt_iop_coloruniformity_params_t *n = calloc(1, sizeof(dt_iop_coloruniformity_params_t));

    n->source_hue=o->source_hue; n->source_chroma=o->source_chroma; n->source_lightness=o->source_lightness;
    n->hue_width=o->hue_width; n->shadow_limit=o->shadow_limit; n->highlight_limit=o->highlight_limit;
    n->target_hue=o->target_hue; n->target_chroma=o->target_chroma; n->target_lightness=o->target_lightness;
    n->strength_h=o->strength_h; n->affinity_h=o->affinity_h; n->preserve_h=o->preserve_h;
    n->strength_c=o->strength_c; n->affinity_c=o->affinity_c; n->preserve_c=o->preserve_c;
    n->strength_l=o->strength_l; n->affinity_l=o->affinity_l; n->preserve_l=o->preserve_l;
    n->offset_h=o->offset_h; n->offset_c=o->offset_c; n->offset_l=o->offset_l;
    n->offsets_weighted=o->offsets_weighted; n->preview_mode=o->preview_mode;
    n->bypass_hue=0; n->bypass_chroma=0; n->bypass_luma=0;
    n->mask_hardness=0.0f; n->mask_feather=0.5f; n->chroma_gate=1.0f;
    // V12 defaults
    n->priority_h=0.0f; n->priority_c=0.0f; n->priority_l=0.0f;
    n->preserve_texture_h=0.0f; n->preserve_texture_c=0.0f; n->preserve_texture_l=0.0f;

    *new_params=n; *new_params_size=sizeof(*n); *new_version=12;
    return 0;
  }

  if(old_version == 10)
  {
    // V10 -> V11: added mask_hardness and mask_feather
    typedef struct {
      float source_hue, source_chroma, source_lightness;
      float hue_width, shadow_limit, highlight_limit;
      float target_hue, target_chroma, target_lightness;
      ...
      int   offsets_weighted;
      int   preview_mode;
      int   bypass_hue, bypass_chroma, bypass_luma;
    } dt_iop_coloruniformity_params_v10_t;

    const dt_iop_coloruniformity_params_v10_t *o = old_params;
    dt_iop_coloruniformity_params_t *n = calloc(1, sizeof(dt_iop_coloruniformity_params_t));
    ...
    n->offset_h=o->offset_h; n->offset_c=o->offset_c; n->offset_l=o->offset_l;
    n->offsets_weighted=o->offsets_weighted; n->preview_mode=o->preview_mode;
    n->bypass_hue=o->bypass_hue; n->bypass_chroma=o->bypass_chroma; n->bypass_luma=o->bypass_luma;
    // New in V11
    n->mask_hardness=0.0f; n->mask_feather=0.5f; n->chroma_gate=1.0f;
    // V12 defaults
    n->priority_h=0.0f; n->priority_c=0.0f; n->priority_l=0.0f;
  ...
  if(old_version == 11)
  {
    // V11 -> V12: added priority_h/c/l and preserve_texture_h/c/l
    typedef struct {
      float source_hue, source_chroma, source_lightness;
      float hue_width, shadow_limit, highlight_limit, mask_hardness, mask_feather;
      float target_hue, target_chroma, target_lightness;
      float strength_h, affinity_h, preserve_h;
      float strength_c, affinity_c, preserve_c;
      float strength_l, affinity_l, preserve_l;
      ...
      int   bypass_hue, bypass_chroma, bypass_luma;
      int   preview_mode;
    } dt_iop_coloruniformity_params_v11_t;

    const dt_iop_coloruniformity_params_v11_t *o = old_params;
    dt_iop_coloruniformity_params_t *n = calloc(1, sizeof(dt_iop_coloruniformity_params_t));

    n->source_hue=o->source_hue; n->source_chroma=o->source_chroma; n->source_lightness=o->source_lightness;
    n->hue_width=o->hue_width; n->shadow_limit=o->shadow_limit; n->highlight_limit=o->highlight_limit;
    n->mask_hardness=o->mask_hardness; n->mask_feather=o->mask_feather; n->chroma_gate=1.0f;
    n->target_hue=o->target_hue; n->target_chroma=o->target_chroma; n->target_lightness=o->target_lightness;
    n->strength_h=o->strength_h; n->affinity_h=o->affinity_h; n->preserve_h=o->preserve_h;
    n->strength_c=o->strength_c; n->affinity_c=o->affinity_c; n->preserve_c=o->preserve_c;
    n->strength_l=o->strength_l; n->affinity_l=o->affinity_l; n->preserve_l=o->preserve_l;
    ...
    n->offsets_weighted=o->offsets_weighted; n->preview_mode=o->preview_mode;
    n->bypass_hue=o->bypass_hue; n->bypass_chroma=o->bypass_chroma; n->bypass_luma=o->bypass_luma;
    // New in V12: neutral values (identical behavior to V11)
    n->priority_h=0.0f; n->priority_c=0.0f; n->priority_l=0.0f;
    n->preserve_texture_h=0.0f; n->preserve_texture_c=0.0f; n->preserve_texture_l=0.0f;
  ...
  n->source_hue = 0.12f; n->source_chroma = 0.20f; n->source_lightness = 0.50f;
  n->hue_width = 0.30f;
  n->shadow_limit = 1.5f;
  n->highlight_limit = 1.5f;
  n->mask_hardness = 0.0f; n->mask_feather = 0.5f; n->chroma_gate = 1.0f;
  n->target_hue = 0.12f; n->target_chroma = 0.20f; n->target_lightness = 0.50f;
  n->strength_h = 0.0f; n->affinity_h = 0.0f; n->preserve_h = 0.50f;
  n->strength_c = 0.0f; n->affinity_c = 0.0f; n->preserve_c = 0.50f;
  n->strength_l = 0.0f; n->affinity_l = 0.0f; n->preserve_l = 0.50f;
  n->offset_h = 0.0f; n->offset_c = 0.0f; n->offset_l = 0.0f;
  n->offsets_weighted = 0;
  n->preview_mode = PREVIEW_NONE;
  n->bypass_hue = 0; n->bypass_chroma = 0; n->bypass_luma = 0;
  n->priority_h = 0.0f; n->priority_c = 0.0f; n->priority_l = 0.0f;
  n->preserve_texture_h = 0.0f; n->preserve_texture_c = 0.0f; n->preserve_texture_l = 0.0f;

  *new_params = n;
  *new_params_size = sizeof(dt_iop_coloruniformity_params_t);
  *new_version = 12;

  return 0;
}

typedef struct dt_iop_coloruniformity_gui_data_t {
  // Selection
  GtkWidget *source_hue_picker;
  GtkWidget *source_hue_slider;
  GtkWidget *source_lightness;
  GtkWidget *hue_width;
  GtkWidget *shadow_limit;
  GtkWidget *highlight_limit;
  GtkWidget *mask_hardness;
  GtkWidget *mask_feather;
  GtkWidget *chroma_gate;
...
} dt_iop_coloruniformity_gui_data_t;


const char *name() { return _("color uniformity"); }
...
// Selection engine -- hard pizza slice + asymmetric luminance window
static inline float compute_affinity_weight(
    const float Y_pix, const float h_pix,
    const float Y_src, const float h_src,
    const float hue_width, const float shadow_limit, const float highlight_limit,
    const float mask_hardness, const float mask_feather)
{
  const float half_angle = fmaxf(hue_width * 0.5f, 1e-6f);
  const float dH = hue_dist(h_pix, h_src);
  const float t_h = fabsf(dH) / half_angle;

  // Hard cutoff beyond the pizza slice
  if(t_h >= 1.0f) return 0.0f;

  float mask_h;
  const float hard = CLAMPF(mask_hardness, 0.0f, 0.999f);
  if(t_h <= hard)
  {
    mask_h = 1.0f;
  }
  ...
  {
    const float u = (t_h - hard) / fmaxf(1.0f - hard, 1e-6f);
    const float ss3 = u * u * (3.0f - 2.0f * u);
    const float ss5 = u * u * u * (u * (u * 6.0f - 15.0f) + 10.0f);
    const float blend = ss3 + mask_feather * (ss5 - ss3);
    mask_h = 1.0f - blend;
  }
  ...
  const float dY = log2f(fmaxf(Y_pix, 1e-6f)) - log2f(fmaxf(Y_src, 1e-6f));
  ...
  if(dY >= 0.0f)
  {
    const float safe_hl = fmaxf(highlight_limit, 1e-3f);
    t_l = dY / safe_hl;
    if(t_l >= 1.0f) return 0.0f;
  }
  else
  {
    const float safe_sl = fmaxf(shadow_limit, 1e-3f);
    t_l = -dY / safe_sl;
    if(t_l >= 1.0f) return 0.0f;
  }
  ...
  {
    const float u = (t_l - hard) / fmaxf(1.0f - hard, 1e-6f);
    const float ss3 = u * u * (3.0f - 2.0f * u);
    const float ss5 = u * u * u * (u * (u * 6.0f - 15.0f) + 10.0f);
    const float blend = ss3 + mask_feather * (ss5 - ss3);
    mask_l = 1.0f - blend;
...
{
  const dt_iop_coloruniformity_params_t *const p = piece->data;
  ...
  for (int j = 0; j < height; j++) {
    ...
    for (int i = 0; i < width; i++, in_ptr += ch, out_ptr += ch) {
      ...
      float weight = compute_affinity_weight(
          Y_pix, h_pix,
          p->source_lightness, p->source_hue,
          p->hue_width, p->shadow_limit, p->highlight_limit,
          p->mask_hardness, p->mask_feather);

      // Hue reliability gate:
      // chroma_gate = 1.0 (default): full gate -- achromatic pixels
      //   achromatic pixels are excluded, preventing hue shifts in gray areas.
      // chroma_gate = 0.0: gate disabled -- all pixels pass through,
      //   including dark/achromatic areas.
      if(p->chroma_gate > 0.0f)
      {
        const float edge = fmaxf(p->source_chroma * 0.5f * p->chroma_gate, 1e-4f);
        const float t = fminf(fmaxf(c_pix / edge, 0.0f), 1.0f);
        weight *= t * t * (3.0f - 2.0f * t);  // smoothstep
      }

      // --- P1: PER-COMPONENT normalized distance for affinity ---
      // Each component uses its own distance to the reference.
      const float t_norm_h = fabsf(hue_dist(h_pix, p->source_hue))
                           / fmaxf(p->hue_width * 0.5f, 1e-6f);
      const float t_norm_c = (p->source_chroma > 1e-4f)
                           ? fabsf(c_pix - p->source_chroma) / p->source_chroma
                           : 0.0f;
      const float dY_aff = log2f(fmaxf(Y_pix, 1e-6f)) - log2f(fmaxf(p->source_lightness, 1e-6f));
      const float t_norm_l = fabsf(dY_aff)
                           / fmaxf(dY_aff >= 0.0f ? p->highlight_limit : p->shadow_limit, 1e-3f);
      ...
      if (!p->bypass_hue) {
        ...
        {
          const float d_norm = fabsf(hue_dist(h_pix, p->target_hue))
                             / fmaxf(p->hue_width * 0.5f, 1e-6f);
      ...
      if (!p->bypass_luma) {
        ...
        {
          const float range = fmaxf(p->shadow_limit + p->highlight_limit, 1e-3f);
...
typedef struct dt_iop_coloruniformity_global_data_t {
  int kernel_coloruniformity_apply;
} dt_iop_coloruniformity_global_data_t;

void init_global(dt_iop_module_so_t *self) {
  dt_iop_coloruniformity_global_data_t *gd = malloc(sizeof(dt_iop_coloruniformity_global_data_t));
  self->data = gd;
  gd->kernel_coloruniformity_apply = dt_opencl_create_kernel(42, "coloruniformity_apply");
}

void cleanup_global(dt_iop_module_so_t *self) {
  dt_iop_coloruniformity_global_data_t *gd = self->data;
  dt_opencl_free_kernel(gd->kernel_coloruniformity_apply);
  free(self->data);
  self->data = NULL;
}
...
{
  const dt_iop_coloruniformity_params_t *const p = piece->data;
  const dt_iop_coloruniformity_global_data_t *const gd = self->global_data;
  cl_int err = DT_OPENCL_DEFAULT_ERROR;
  if(piece->colors != 4) return err;

  // Visual preview modes (DELTA, DELTA_MASK, MASK, OVERLAY) are implemented
  // only in process() CPU path. Fall back to CPU so the preview renders correctly.
  if(p->preview_mode != PREVIEW_NONE)
  ...
  dt_colormatrix_t inm, outm;
  dt_colormatrix_mul(inm,  XYZ_D50_to_D65_CAT16, wp->matrix_in);
  dt_colormatrix_mul(outm, wp->matrix_out, XYZ_D65_to_D50_CAT16);

  cl_mem inm_cl  = dt_opencl_alloc_device_buffer(devid, sizeof(dt_colormatrix_t));
  cl_mem outm_cl = dt_opencl_alloc_device_buffer(devid, sizeof(dt_colormatrix_t));
  cl_mem xyz_to_lms_cl = dt_opencl_alloc_device_buffer(devid, sizeof(dt_colormatrix_t));
  cl_mem lms_to_xyz_cl = dt_opencl_alloc_device_buffer(devid, sizeof(dt_colormatrix_t));

  if(!inm_cl || !outm_cl || !xyz_to_lms_cl || !lms_to_xyz_cl) { err = CL_MEM_OBJECT_ALLOCATION_FAILURE; goto error_m; }

  err = (darktable.opencl->dlocl->symbols->dt_clEnqueueWriteBuffer)(
      darktable.opencl->dev[devid].cmd_queue, inm_cl, CL_TRUE,
      0, sizeof(dt_colormatrix_t), inm, 0, NULL, NULL);
  if(err != CL_SUCCESS) goto error_m;

  err = (darktable.opencl->dlocl->symbols->dt_clEnqueueWriteBuffer)(
      darktable.opencl->dev[devid].cmd_queue, outm_cl, CL_TRUE,
      0, sizeof(dt_colormatrix_t), outm, 0, NULL, NULL);
  if(err != CL_SUCCESS) goto error_m;

  err = (darktable.opencl->dlocl->symbols->dt_clEnqueueWriteBuffer)(
      darktable.opencl->dev[devid].cmd_queue, xyz_to_lms_cl, CL_TRUE,
      0, sizeof(dt_colormatrix_t), XYZ_D65_to_LMS_2006_D65, 0, NULL, NULL);
  if(err != CL_SUCCESS) goto error_m;

  err = (darktable.opencl->dlocl->symbols->dt_clEnqueueWriteBuffer)(
      darktable.opencl->dev[devid].cmd_queue, lms_to_xyz_cl, CL_TRUE,
      0, sizeof(dt_colormatrix_t), LMS_2006_D65_to_XYZ_D65, 0, NULL, NULL);
  if(err != CL_SUCCESS) goto error_m;

  // Apply bypass: zero out strengths/offsets before sending to kernel
  const float cl_strength_h = p->bypass_hue    ? 0.0f : p->strength_h;
  const float cl_affinity_h = p->bypass_hue    ? 0.0f : p->affinity_h;
  const float cl_offset_h   = p->bypass_hue    ? 0.0f : p->offset_h;
  const float cl_priority_h = p->bypass_hue    ? 0.0f : p->priority_h;
  const float cl_preserve_texture_h = p->bypass_hue ? 0.0f : p->preserve_texture_h;

  const float cl_strength_c = p->bypass_chroma ? 0.0f : p->strength_c;
  const float cl_affinity_c = p->bypass_chroma ? 0.0f : p->affinity_c;
  const float cl_offset_c   = p->bypass_chroma ? 0.0f : p->offset_c;
  const float cl_priority_c = p->bypass_chroma ? 0.0f : p->priority_c;
  const float cl_preserve_texture_c = p->bypass_chroma ? 0.0f : p->preserve_texture_c;

  const float cl_strength_l = p->bypass_luma   ? 0.0f : p->strength_l;
  const float cl_affinity_l = p->bypass_luma   ? 0.0f : p->affinity_l;
  const float cl_offset_l   = p->bypass_luma   ? 0.0f : p->offset_l;
  const float cl_priority_l = p->bypass_luma   ? 0.0f : p->priority_l;
  const float cl_preserve_texture_l = p->bypass_luma ? 0.0f : p->preserve_texture_l;

  err = dt_opencl_enqueue_kernel_2d_args(
      devid, gd->kernel_coloruniformity_apply, width, height,
      CLARG(dev_in), CLARG(dev_out),
      CLARG(width), CLARG(height),
      CLARG(inm_cl), CLARG(outm_cl),
      CLARG(xyz_to_lms_cl), CLARG(lms_to_xyz_cl),
      CLARG(p->source_hue), CLARG(p->source_chroma), CLARG(p->source_lightness),
      CLARG(p->hue_width), CLARG(p->shadow_limit), CLARG(p->highlight_limit),
      CLARG(p->mask_hardness), CLARG(p->mask_feather), CLARG(p->chroma_gate),
      CLARG(p->target_hue), CLARG(p->target_chroma), CLARG(p->target_lightness),
      CLARG(cl_strength_h), CLARG(cl_affinity_h), CLARG(p->preserve_h),
      CLARG(cl_strength_c), CLARG(cl_affinity_c), CLARG(p->preserve_c),
      CLARG(cl_strength_l), CLARG(cl_affinity_l), CLARG(p->preserve_l),
      CLARG(cl_offset_h), CLARG(cl_offset_c), CLARG(cl_offset_l),
      CLARG(p->offsets_weighted),
      CLARG(p->preview_mode), CLARG(Lwhite),
      /* V12 */ CLARG(cl_priority_h), CLARG(cl_priority_c), CLARG(cl_priority_l),
      CLARG(cl_preserve_texture_h), CLARG(cl_preserve_texture_c), CLARG(cl_preserve_texture_l));

error_m:
  dt_opencl_release_mem_object(lms_to_xyz_cl);
  dt_opencl_release_mem_object(xyz_to_lms_cl);
  dt_opencl_release_mem_object(outm_cl);
  dt_opencl_release_mem_object(inm_cl);
  return err;
}
#endif
...
static void _shift_hue_cb(GtkWidget *widget, gpointer user_data)
{
  ...
  dt_iop_coloruniformity_params_t *p = (dt_iop_coloruniformity_params_t *)self->params;
  dt_iop_coloruniformity_gui_data_t *g = (dt_iop_coloruniformity_gui_data_t *)self->gui_data;
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
  ...
  if(picker == g->source_hue_picker) {
    ...
    dt_bauhaus_slider_set(g->source_hue_slider, p->source_hue);
    dt_bauhaus_slider_set(g->source_lightness, p->source_lightness);
...
static void _slider_changed_cb(GtkWidget *widget, gpointer user_data) {
  ...
  // Selection
  if (widget == g->source_hue_slider) p->source_hue = val;
  else if (widget == g->source_lightness) p->source_lightness = val;
  else if (widget == g->hue_width) p->hue_width = val;
  else if (widget == g->shadow_limit) p->shadow_limit = val;
  else if (widget == g->highlight_limit) p->highlight_limit = val;
  else if (widget == g->mask_hardness)   p->mask_hardness   = val;
  else if (widget == g->mask_feather)    p->mask_feather    = val;
  else if (widget == g->chroma_gate)     p->chroma_gate     = val;
  // Correction
...
{
  dt_iop_module_t *self = (dt_iop_module_t *)user_data;
  dt_iop_coloruniformity_params_t *p = self->params;
...
void gui_update(dt_iop_module_t *self) {
  dt_iop_coloruniformity_gui_data_t *g = self->gui_data;
  dt_iop_coloruniformity_params_t *p = self->params;
  ++darktable.gui->reset;
  ...
  dt_bauhaus_slider_set(g->source_hue_slider, p->source_hue);
  dt_bauhaus_slider_set(g->source_lightness, p->source_lightness);
  dt_bauhaus_slider_set(g->hue_width, p->hue_width);
  dt_bauhaus_slider_set(g->shadow_limit, p->shadow_limit);
  dt_bauhaus_slider_set(g->highlight_limit, p->highlight_limit);
  dt_bauhaus_slider_set(g->mask_hardness,   p->mask_hardness);
  dt_bauhaus_slider_set(g->mask_feather,    p->mask_feather);
  dt_bauhaus_slider_set(g->chroma_gate,     p->chroma_gate);
  // Correction
  dt_bauhaus_slider_set(g->target_hue_slider, p->target_hue);
  dt_bauhaus_slider_set(g->target_chroma, p->target_chroma);
  dt_bauhaus_slider_set(g->target_lightness, p->target_lightness);
...
{
  dt_iop_coloruniformity_gui_data_t *g = IOP_GUI_ALLOC(coloruniformity);
  ...
  g->source_hue_slider = _create_manual_slider(self, _("hue"),
                                               0.0f, 1.0f, 0.001f, 0.12f, 2, "°", 360.0f);
  dt_bauhaus_slider_set_feedback(g->source_hue_slider, 0);
  _paint_hue_slider(g->source_hue_slider);
  g->source_hue_picker = dt_color_picker_new(self, DT_COLOR_PICKER_AREA, g->source_hue_slider);
  dt_gui_box_add(page_sel, g->source_hue_picker);

  dt_gui_box_add(page_sel, dt_ui_section_label_new(_("Selection Range")));

  g->hue_width = _create_manual_slider(self, _("hue width"),
                                       0.02f, 1.0f, 0.01f, 0.30f, 0, "°", 360.0f);
  dt_gui_box_add(page_sel, g->hue_width);

  g->source_lightness = _create_manual_slider(self, _("source lightness"),
                                              0.0f, 1.5f, 0.001f, 0.50f, 2, "%", 100.0f);
  _paint_luma_slider(g->source_lightness);
  dt_gui_box_add(page_sel, g->source_lightness);

  g->shadow_limit = _create_manual_slider(self, _("shadow limit"),
                                          0.1f, 12.0f, 0.1f, 1.5f, 1, " EV", 1.0f);
  dt_gui_box_add(page_sel, g->shadow_limit);

  g->highlight_limit = _create_manual_slider(self, _("highlight limit"),
                                             0.1f, 12.0f, 0.1f, 1.5f, 1, " EV", 1.0f);
  dt_gui_box_add(page_sel, g->highlight_limit);

  dt_gui_box_add(page_sel, dt_ui_section_label_new(_("Mask Quality")));

  g->mask_hardness = _create_manual_slider(self, _("flat zone"),
                                           0.0f, 1.0f, 0.01f, 0.0f, 2, "%", 100.0f);
  gtk_widget_set_tooltip_text(g->mask_hardness,
    _("central fraction at full weight\n"
      "0% = transition over full width (biweight)\n"
      "50% = flat center, transition on outer half\n"
      "100% = quasi-boolean selection"));
  dt_gui_box_add(page_sel, g->mask_hardness);

  g->mask_feather = _create_manual_slider(self, _("transition softness"),
                                          0.0f, 1.0f, 0.01f, 0.5f, 2, "%", 100.0f);
  gtk_widget_set_tooltip_text(g->mask_feather,
    _("transition shape towards mask edge\n"
      "0% = smoothstep (degree 3)\n"
      "100% = smootherstep (degree 5, softer edges)"));
  dt_gui_box_add(page_sel, g->mask_feather);

  g->chroma_gate = _create_manual_slider(self, _("achromatic protection"),
                                         0.0f, 1.0f, 0.01f, 1.0f, 2, "%", 100.0f);
  gtk_widget_set_tooltip_text(g->chroma_gate,
    _("protects gray/achromatic areas from hue shifts\n"
      "100% = full protection (default) -- low-chroma pixels excluded\n"
      "0% = no protection -- all luminances included,\n"
      "including very dark or gray areas"));
  dt_gui_box_add(page_sel, g->chroma_gate);
  ...
  // === Bypass bar: Hue / Chroma / Lightness ===
  {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->btn_bypass_hue),
      ((const dt_iop_coloruniformity_params_t *)self->params)->bypass_hue);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->btn_bypass_chroma),
      ((const dt_iop_coloruniformity_params_t *)self->params)->bypass_chroma);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->btn_bypass_luma),
      ((const dt_iop_coloruniformity_params_t *)self->params)->bypass_luma);
